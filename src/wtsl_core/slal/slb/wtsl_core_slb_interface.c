...
int init_node() 
{
	...
	ret = glob("/tmp/AutoScripts_*.sh", GLOB_TILDE | GLOB_ERR, NULL, &glob_buf);
	if (ret == 0) {
		matched_file = glob_buf.gl_pathv[0];
		WTSL_LOG_INFO(MODULE_NAME, "find /tmp/%s", matched_file);

		if (strstr(matched_file, "AutoScripts_hi.sh") != NULL) {
			WTSL_LOG_DEBUG(MODULE_NAME, "exec hi2921 arch platform compatible");
			char command[64];
			snprintf(command, sizeof(command), "chmod +x /tmp/AutoScripts_hi.sh && /tmp/AutoScripts_hi.sh hi");
			int ret_hi = system(command);
			if (ret_hi != 0) {
				WTSL_LOG_ERROR(MODULE_NAME, "exec AutoScripts_hi.sh failed");
			} else {
				WTSL_LOG_INFO(MODULE_NAME, "exec AutoScripts_hi.sh success");
			}
		} else if (strstr(matched_file, "AutoScripts_rk.sh") != NULL) {
			WTSL_LOG_DEBUG(MODULE_NAME, "exec rk3568 arch platform compatible");
			char command[64];
			snprintf(command, sizeof(command), "chmod +x /tmp/AutoScripts_rk.sh && /tmp/AutoScripts_rk.sh rk");
			int ret_rk = system(command);
			if (ret_rk != 0) {
				WTSL_LOG_ERROR(MODULE_NAME, "exec AutoScripts_rk.sh failed");
			} else {
				WTSL_LOG_INFO(MODULE_NAME, "exec AutoScripts_rk.sh success");
			}
		}
	} else if (ret == GLOB_NOMATCH) {
		WTSL_LOG_DEBUG(MODULE_NAME, "not found AutoScripts_*.sh shell file");
		char command[64];
		snprintf(command, sizeof(command), "chmod +x /tmp/AutoScripts*.sh && /tmp/AutoScripts*.sh default");
		int ret_default = system(command);
		if (ret_default != 0) {
			WTSL_LOG_ERROR(MODULE_NAME, "exec AutoScripts.sh default failed");
		} else {
			WTSL_LOG_INFO(MODULE_NAME, "exec AutoScripts.sh default success");
		}
	}
	...
	slb_set_fisa_enable(NET_VAP_NAME, adv_info->fisa_enable);
...
int slb_set_fisa_enable(char *dev_name, int enable)
{
	int ret = 0;
	//参数验证
	if(dev_name == NULL || strlen(dev_name) == 0 || strlen(dev_name) > MAX_DEVICE_NAME_LEN)
	...
	if(enable)
	{
		//只支持在G节点使用，而且只有业务带宽小于物理带宽时才能正常使用
		if((slb_get_tfc_bw(dev_name)) >=  (slb_get_bw(dev_name)))
			WTSL_LOG_WARNING(MODULE_NAME, "warning: trc_bw must be less than bw");
		snprintf(cmd, sizeof(cmd), "iwpriv %s alg \"fisa enable\"", NET_VAP_NAME);
		ret = system(cmd);
	}
	else
	{
		snprintf(cmd, sizeof(cmd), "iwpriv %s alg \"fisa disable\"", NET_VAP_NAME);
		ret = system(cmd);
	}

	if(ret != 0)
	{
		WTSL_LOG_ERROR(MODULE_NAME, "Error: Failed to set fisa %d", enable);
		return -1;
	}
...
*/
	...
	if (node_params->type == NODE_TYPE_SLB_T) {
		slb_set_sec_exch_cap(NET_VAP_NAME, adv_info->sec_exch_cap);
	}
