...
/**
 * @if Eng
 * @brief Enum of sle add method or event.
 * @else
 * @brief 添加方法或事件标识。
 * @endif
 */
typedef enum {
    SLE_SSAPS_ADD_METHOD              = 0x00,    /*!< @if Eng SSAPS add method
                                                      @else   服务端添加事件 @endif */
    SLE_SSAPS_ADD_EVENT               = 0x01,    /*!< @if Eng SSAPS add event
                                                      @else   服务端添加方法 @endif */
} sle_ssaps_add_method_event_type_t;
...
/**
 * @if Eng
 * @brief Struct of send method response information.
 * @else
 * @brief 发送方法响应信息。
 * @endif
 */
...
/**
 * @if Eng
 * @brief Struct of send event notification/indication information.
 * @else
 * @brief 事件发送通知/指示信息。
 * @endif
 */
typedef struct {
    uint16_t handle;      /*!< @if Eng Event handle.
                               @else   属性句柄。 @endif */
    bool need_rsp;        /*!< @if Eng The receiver needs to respond.
                               @else   需要接收端回复响应。 @endif */
    uint16_t value_len;   /*!< @if Eng Length of notification/indication data.
                               @else   通知/指示数据长度。 @endif */
    uint8_t *value;       /*!< @if Eng Notification/indication data.
                               @else   发送的通知/指示数据。 @endif */
} ssaps_event_ntf_ind_t;
...
/**
 * @if Eng
 * @brief  Add a ssap method or event.
 * @par Description: Add a ssap method or event.
 * @param  [in]  server_id      Server ID.
 * @param  [in]  service_handle Service handle.
 * @param  [in]  param          SSAP method or event Info.
 * @param  [in]  type           SSAP method or event type.
 * @param  [out] handle         method or event handle.
 * @retval error code.
 * @par Depends:
 * @li sle_ssap_stru.h
 * @else
 * @brief  添加一个ssap方法或事件。
 * @par Description: 添加一个ssap方法或事件。
 * @param  [in]  server_id      服务端 ID。
 * @param  [in]  service_handle 服务句柄。
 * @param  [in]  param          SSAP 方法或事件信息。
 * @param  [in]  type           SSAP 方法或事件类别。
 * @param  [out] handle         方法或事件句柄
 * @retval 执行结果错误码。
 * @par 依赖：
 * @li sle_ssap_stru.h
 * @endif
 */
errcode_t ssaps_add_method_event_sync(uint8_t server_id, uint16_t service_handle, ssaps_property_info_t *param,
...
/**
 * @if Eng
 * @brief  Send call method response when receive the request need to response by user.
 * @par Description: Send call method response when receive the request need to response by user
                     { @ref ssaps_call_method_request_callback }.
 * @param  [in] server_id server ID.
 * @param  [in] conn_id   connection ID.
 * @param  [in] param     method response parameter.
 * @retval error code.
 * @par Depends:
 * @li sle_ssap_stru.h
 * @else
 * @brief  当收到需要用户回复响应的请求时发送方法调用响应。
 * @par Description: 当收到需要用户回复响应的请求时发送方法调用响应
                     { @ref ssaps_call_method_request_callback }。
 * @param  [in] server_id 服务端 ID.
 * @param  [in] conn_id   连接ID。
 * @param  [in] param     方法响应参数.
 * @retval 执行结果错误码。
 * @par 依赖：
 * @li sle_ssap_stru.h
 * @endif
 */
errcode_t ssaps_send_method_response(uint8_t server_id, uint16_t conn_id, ssaps_send_method_rsp_t *param);
...
/**
 * @if Eng
 * @brief  Send event indication or notification to remote device.
 * @par Description: Send event indication or notification to remote device.
                     send status depend on need_rsp param.
 * @param  [in] server_id server ID.
 * @param  [in] conn_id   connection ID，To send packets to all peer ends, enter conn_id = 0xffff.
 * @param  [in] param     notify/indicate parameter.
 * @retval error code.
 ...
 * @else
 * @brief  向对端发送事件通知或指示。
 * @par Description: 向对端发送事件通知或指示，具体发送状态取决于参数need_rsp字段
 * @param  [in] server_id 服务端 ID。
 * @param  [in] conn_id   连接ID，如果向全部对端发送则输入conn_id = 0xffff；
 * @param  [in] param     通知或指示参数。
 * @retval 执行结果错误码。
 ...
 * @endif
 */
errcode_t ssaps_event_notify_indicate(uint8_t server_id, uint16_t conn_id, ssaps_event_ntf_ind_t *param);
...
#endif
#include "sle_ssap_server_inner.h"
#endif
/** @} */ 
