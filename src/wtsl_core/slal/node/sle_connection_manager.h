...
/**
 * @if Eng
 * @brief Out-of-band data flag.
 * @else
 * @brief 带外数据标识。
 * @endif
 */
typedef enum {
    SLE_OOB_DATA_DISABLE,            /*!< @if Eng Disable Out-Of-Band.
                                          @else 无带外数据。 @endif */
    SLE_OOB_DATA_ENABLE,             /*!< @if Eng Enable Out-Of-Band.
                                          @else 有带外数据。 @endif */
} sle_pair_oob_flag_t;
 
/**
 * @if Eng
 * @brief Pre-Shared Key data flag.
 * @else
 * @brief 预置共享密钥数据标识。
 * @endif
 */
typedef enum {
    SLE_PSK_DATA_DISABLE,            /*!< @if Eng Disable Out-Of-Band.
                                          @else 无PSK数据。 @endif */
    SLE_PSK_DATA_ENABLE,             /*!< @if Eng Enable Out-Of-Band.
                                          @else 有PSK数据。 @endif */
} sle_pair_psk_flag_t;
...
typedef struct {
    ...
    uint8_t oob_flag;            /*!< @if Eng Out-Of-Band data flag { @ref sle_pair_oob_flag_t }
                                    @else   带外数据标识 { @ref sle_pair_oob_flag_t } @endif */
    uint8_t psk_flag;            /*!< @if Eng Pre-Shared Key data flag { @ref sle_pair_psk_flag_t }
                                    @else   预置共享密钥数据标识 { @ref sle_pair_psk_flag_t } @endif */
} sle_sm_secu_param_t;
 ...
 * 当连接状态发生变化时，该回调函数被调用。
 * @attention
 * 1. 该函数在SLE服务上下文中被调用，不应被阻塞或执行长时间等待的操作。
 * 2. 指针参数的内存由SLE服务自动申请和释放。
 * @param [in] conn_id    连接ID，用于标识具体的连接。
 * @param [in] addr       地址信息，表示连接的对端地址。
 * @param [in] conn_state 连接状态，具体定义见 @ref sle_acb_state_t。
 * @param [in] role       连接角色，具体定义见 @ref sle_link_role_t。
 * @retval errcode_t 返回错误码，具体定义见 errcode_t。
 * @par 依赖
 * @li sle_common.h
 * @see sle_connection_callbacks_t
 */
typedef errcode_t (*sle_connect_status_changed_callback)(uint16_t conn_id, const sle_addr_t *addr,
    sle_acb_state_t conn_state, sle_link_role_t role);
...
typedef void (*sle_sync_timer_callback)(uint16_t conn_id, errcode_t status, const sle_sync_timer_t *param);
...
typedef struct {
    ...
    sle_sync_timer_callback sync_timer_cb;                               /*!< @if Eng sync timer callback.
                                                                            @else   设置时钟同步回调函数。 @endif */
 ...
 * @endif
 */
errcode_t sle_set_secu_param(sle_sm_secu_param_t* sec_params);
...
/**
 * @if Eng
 * @brief  recover nv smp keys.
 * @par Description: recover nv smp keys.
 * @retval error code.
 * @par Depends:
 * @li sle_common.h
 * @else
 * @brief  从nv回复秘钥信息。
 * @par Description: 从nv回复秘钥信息。
 * @retval 执行结果错误码。
 * @par 依赖：
 * @li sle_common.h
 * @endif
 */
errcode_t sle_recover_nv_smp_keys(void);
