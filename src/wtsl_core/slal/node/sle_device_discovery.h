...
typedef enum {
    ...
    SLE_MCS_02,                                         /*!< @if Eng MCS2:  BPSK1/4
                                                             @else   MCS2:  BPSK1/4 @endif */
...
typedef struct sle_seek_params {
    ...
    uint8_t seek_    filter_policy;                   /*!< @if Eng scan filter policy { @ref sle_seek_filter_t }
                                                       @else   扫描设备使用的过滤类型，
...
typedef struct {
    ...
    sle_seek_result_callback seek_result_cb;                /*!< @if Eng scan result callback.
                                                                 @else   扫描结果回调函数。 @endif */
    sle_dfr_callback sle_dfr_cb;                                /*!< @if Eng General callback foo sle dfr.
                                                                 @else   dfr回调函数。 @endif */
} sle_device_discovery_callbacks_t;
