...
static cJSON *create_node_info_json(const WTSLNode *node) {
    ...
    cJSON_AddItemToObject(item, "aj_flag", cJSON_CreateNumber(node->info.basic_info.auto_join_net_flag));
    return item;
}
...
void* wtsl_core_do_throughput(void* ph, void *data, unsigned int size, UserContext *ctx)
{
	...
	snprintf(cmd, sizeof(cmd), "iwpriv %s alg \"set_mcs_bound 00:00:00:00:00:00 6 30\"", NET_VAP_NAME);
	system(cmd);
...
void* wtsl_core_get_sle_basicinfo(void* ph, void *data, unsigned int size, UserContext *ctx)
{
	...
	cJSON_AddItemToObject(root, "sle_enable", cJSON_CreateNumber(global_node_info.node_info.basic_info.sle_enable));
...
void* wtsl_core_set_sle_basicinfo(void* ph, void *data, unsigned int size, UserContext *ctx)
{
	...
	cJSON* sle_enable = cJSON_GetObjectItem(root,"sle_enable");
	if(sle_enable != NULL)
	{
		global_node_info.node_info.basic_info.sle_enable = sle_enable->valueint;
		config_set_int("SLE_ENABLE", global_node_info.node_info.basic_info.sle_enable);
		if(global_node_info.node_info.basic_info.sle_enable)
			enable_sle();
		else  
			disable_sle();
	}
...
void* wtsl_core_config_vap0_mac(void* ph, void *data, unsigned int size, UserContext *ctx)
{
	...
	cJSON* cjson_vap0_mac = cJSON_GetObjectItem(root, "vap0_mac");
	if(cjson_vap0_mac != NULL){
		char vap0_mac[18] = {0};
		char cmd[128] = {0};
		strcpy(vap0_mac, cjson_vap0_mac->valuestring);
		system("nanddump -n -l 4096 /dev/mtd10 | strings > /tmp/enva.txt");
		int ret = system("cat /tmp/enva.txt | grep \"vap0_mac\"");
		if(ret == 0)
		{
			//找到vap0_mac,选择更新vap0_mac
			snprintf(cmd, sizeof(cmd), "cat /tmp/enva.txt | strings | sed 's/^vap0_mac=.*/vap0_mac=%s/' > /tmp/enva_temp.txt", vap0_mac);
			printf("cmd:%s\n", cmd);
			system(cmd);
			system("mv /tmp/enva_temp.txt /tmp/enva.txt");			
		}
		else
		{
			//未设置vap0_mac，在尾部追加vap0_mac
			snprintf(cmd, sizeof(cmd), "echo vap0_mac=%s >> /tmp/enva.txt", vap0_mac);
			system(cmd);
		}
		system("flash_erase /dev/mtd10 0 0");
		usleep(500000);
		system("nandwrite -p /dev/mtd10 /tmp/enva.txt");
