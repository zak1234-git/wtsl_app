...
typedef enum {
    SLE_LOW_LATENCY_3HZ   = 3,      /*!< @if Eng 3HZ
                                         @else 3HZ 调度,建议在standby状态下启用。 @endif */
    SLE_LOW_LATENCY_10HZ   = 10,    /*!< @if Eng 10HZ.
                                         @else 10HZ 调度，建议在sleep状态下启用。 @endif */
...
typedef uint8_t *(*sle_low_latency_hid_data_callback)(uint8_t *length, uint16_t *ssap_handle,
    uint8_t *data_type, uint16_t conn_id);
 ...
 * @endif
 */
typedef void (*sle_low_latency_set_em_data_callback)(uint16_t conn_id, uint8_t status);
 ...
 * @endif
 */
errcode_t sle_low_latency_set_em_data(uint16_t conn_id, uint8_t enable);
