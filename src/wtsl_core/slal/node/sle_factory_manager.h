 ...
 */
typedef struct {
    uint16_t tx_freq;            /*!< @if Eng tx frequency,Scope:0x00~0x4E,2402+x
 ...
 */
typedef struct {
    uint16_t rx_freq;            /*!< @if Eng rx frequency,Scope:0x00~0x4E
                                     @else   发送频率,范围:0x00~0x4E @endif */
    uint8_t rx_phy;             /*!< @if Eng radio frequency physical { @ref sle_rf_phy_t }
                                     @else   射频物理层 { @ref sle_rf_phy_t } @endif */
    uint8_t rx_format;          /*!< @if Eng rx format { @ref sle_rf_format_t }
                                     @else   发射格式 { @ref sle_rf_format_t } @endif */
...
typedef struct {
    uint8_t status;             /*!< @if Eng event complete status
                                     @else   事件完成状态 @endif */
    uint16_t num_packets;       /*!< @if Eng number packets
                                     @else   数据包数 @endif */
