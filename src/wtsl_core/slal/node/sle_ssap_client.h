...
/**
 * @if Eng
 * @brief Method/Event discovery result.
 * @else
 * @brief 方法、事件发现结果。
 * @endif
 */
typedef struct {
    uint16_t   handle;     /*!< @if Eng method handle.
                                @else   句柄。 @endif */
    sle_uuid_t uuid;       /*!< @if Eng UUID.
                                @else   UUID。 @endif */
} ssapc_find_method_event_result_t;
 ...
 */
typedef struct {
    uint8_t    type;        /*!< @if Eng find type { @ref ssap_property_type_t }.
                                 @else   查找类型 { @ref ssap_property_type_t }。 @endif */
    uint16_t   start_hdl;   /*!< @if Eng start handle.
                                 @else   起始句柄。@endif */
    uint16_t   end_hdl;     /*!< @if Eng end handle.
                                 @else   结束句柄。@endif */
...
/**
 * @if Eng
 * @brief Callback invoked when method or event discovered.
 * @par Callback invoked when method or event discovered.
 * @attention 1.This function is called in SLE service context,should not be blocked or do long time waiting.
 * @attention 2.The memories of pointer are requested and freed by the SLE service automatically.
 * @param [in] client_id client ID.
 * @param [in] conn_id   connection ID.
 * @param [in] param     method or event param.
 * @param [in] type      method or event type.
 * @param [in] status    error code.
 * @retval #void no return value.
 * @par Dependency:
 * @li  sle_common.h
 ...
 * @param [in] client_id 客户端 ID。
 * @param [in] conn_id   连接 ID。
 * @param [in] param     方法或事件结构。
 * @param [in] type      方法或事件类别。
 * @param [in] status    执行结果错误码。
 * @retval 无返回值。
 * @par 依赖:
 * @li  sle_common.h
 ...
 * @endif
 */
typedef void (*ssapc_find_method_event_callback)(uint8_t client_id, uint16_t conn_id,
    ssapc_find_method_event_result_t *param, uint8_t type, errcode_t status);
 ...
 * @attention 1.This function is called in SLE service context,should not be blocked or do long time waiting.
 * @attention 2.The memories of pointer are requested and freed by the SLE service automatically.
 * @attention 3.The handle and type of read_data are not specified in the protocol.
 * @attention   They are reserved parameters but not read_req parameters.
 * @param [in] client_id client ID.
 * @param [in] conn_id   connection ID.
 ...
 * @attention  1. 该回调函数运行于SLE service线程，不能阻塞或长时间等待。
 * @attention  2. pointer由SLE service申请内存，也由SLE service释放，回调中不应释放。
 * @attention  3. 当前read_data的handle和type协议中未规定使用，作为预留参数，非read_req的参数。
 * @param [in] client_id 客户端 ID。
 * @param [in] conn_id   连接 ID。
...
/**
 * @if Eng
 * @brief Callback function for receiving call method response.
 * @par Callback function for receiving call method response.
 * @attention 1.This function is called in SLE service context,should not be blocked or do long time waiting.
 * @attention 2.The memories of pointer are requested and freed by the SLE service automatically.
 * @param [in] client_id    client ID.
 * @param [in] conn_id      connection ID.
 * @param [in] call_method_result call method result.
 * @retval #void no return value.
 * @par Dependency:
 * @li  sle_common.h
 * @see ssapc_callbacks_t
 ...
 * @param [in] client_id    客户端 ID。
 * @param [in] conn_id      连接 ID。
 * @param [in] call_method_result 方法调用结果。
 * @retval 无返回值。
 * @par 依赖:
 * @li  sle_common.h
 * @see ssapc_callbacks_t
 * @endif
 */
typedef void (*ssapc_call_method_cfm_callback)(uint8_t client_id, uint16_t conn_id,
    ssapc_method_param_t *call_method_result);
...
typedef struct {
    ...
    ssapc_indication_callback indication_cb;                      /*!< @if Eng Callback hook for receive indication.
                                                                       @else   指示事件上报钩子。 @endif */
    ssapc_find_method_event_callback ssapc_find_method_event_cbk; /*!< @if Eng Discovery method callback.
                                                                       @else   发现方法或事件回调函数。 @endif */
    ssapc_call_method_cfm_callback call_method_cfm_cb;            /*!< @if Eng Receive call method response callback.
                                                                       @else   收到方法调用响应回调函数。 @endif */
} ssapc_callbacks_t;
 ...
 * @li  sle_common.h
 * @else
 * @brief  查找服务、特征、描述符、方法、事件。
 * @par Description: 查找服务、特征、描述符、方法、事件。
 * @param [in] client_id 客户端 ID。
 * @param [in] conn_id   连接 ID。
 * @param [in] param     查找参数。
...
/**
 * @if Eng
 * @brief  Call method request.
 * @par Description: Call method request.
 * @param [in] client_id Client ID.
 * @param [in] conn_id   Connection ID.
 * @param [in] param     Parameter for call method.
 * @retval error code.
 * @par Depends:
 * @li  sle_common.h
 * @else
 * @brief  方法调用请求。
 * @par Description: 方法调用请求。
 * @param [in] client_id 客户端 ID。
 * @param [in] conn_id   连接 ID。
 * @param [in] param     方法调用参数。
 * @retval 执行结果错误码。
 * @par 依赖：
 * @li  sle_common.h
 * @endif
 */
errcode_t ssapc_call_method_req(uint8_t client_id, uint16_t conn_id, ssapc_method_param_t *param);
 
/**
 * @if Eng
 * @brief  Call method command.
 * @par Description: Call method command.
 * @param [in] client_id Client ID.
 * @param [in] conn_id   Connection ID.
 * @param [in] param     Parameter for call method.
 * @retval error code.
 ...
 * @endif
 */
errcode_t ssapc_call_method_cmd(uint8_t client_id, uint16_t conn_id, ssapc_method_param_t *param);
