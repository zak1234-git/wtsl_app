...
static int upgrade_hi_firmware(const firmware_header_t *header, const char *extract_dir, const char *version_number)
{
    ...
    value = extract_upgrade_version(header->version);
    if (value == 123) {
        char old_cmd[CMD_SIZE] = {0};
        snprintf(old_cmd, sizeof(old_cmd), "cp -rf %s/firmware_t%s /home/ && ln -s /home/firmware_t%s /home/firmware", extract_dir, version_number, version_number);
        system(old_cmd);
    }
    if(ret != 0){
...
static int upgrade_web_files_in_background(const firmware_header_t *header, const char *extract_dir)
{
    ...
    if (access(upgrade_www_dir, F_OK) != 0) {
        ...
        char copy_cmd[CMD_SIZE] = {0};
        snprintf(copy_cmd, sizeof(copy_cmd), "rm -rf /home/wt/www && cp -rf %s /home/wt/", upgrade_www_dir);
...
int parse_firmware_header(const char *json_data, firmware_header_t *header) {
    ...
    cJSON *hostinfo = cJSON_GetObjectItem(root, "hostinfo");
    ...
    if (hostinfo && cJSON_IsString(hostinfo)) {
        strncpy(header->hostinfo, hostinfo->valuestring, sizeof(header->hostinfo)-1);
    }
...
int extract_upgrade_version(const char *version)
{
    char *start, *end;
    int value;

    start = strchr(version,'_');
    if (start == NULL) {
        return -1;
    }
    start++;
...
int Platform_verify(firmware_header_t *header) {
    ...
    WTSL_LOG_DEBUG(MODULE_NAME, "upgrade version num: %d", value);
    // 参数校验
    if (header->targetarch[0] == '\0') {
        if (value >= 123) {
            WTSL_LOG_DEBUG(MODULE_NAME, "upgrade version Greater than or equal to 123");
            return 0;
        } else {
            return -1;
        }
    ...
    int is_arm64 = (strcmp(machine, "aarch64") == 0);
