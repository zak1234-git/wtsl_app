...
int wtsl_core_parse_json_set_node_advinfo(const char *args){
	...
	cJSON *fisa_enable = cJSON_GetObjectItem(root, "fisa_enable");
	if(fisa_enable != NULL)
		adv_info->fisa_enable = fisa_enable->valueint;
	...
	config_set_int("FISA_ENABLE", adv_info->fisa_enable);
...
void* wtsl_core_firmware_upgrade(void *args)
{
	...
	if (strcmp(header.hostinfo, "rk3568") == 0) {
		char command[64];
		snprintf(command, sizeof(command), "/tmp/AutoScripts_rk.sh rk upgrade");
		int ret_rk = system(command);
		if (ret_rk != 0) {
			WTSL_LOG_ERROR(MODULE_NAME, "upgrade %s handle failed", header.hostinfo);
			return strdup("{\"status\":\"error\",\"message\":\"Upgrade process for rk3568 failed\"}");
		}
	} else {
